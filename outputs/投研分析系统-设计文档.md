...
期货真实数据接入复用统一的 `data-source-refresh.v1`，不在商品适配器内重复实现一套网络客户端。对已保存的刷新结果，使用 `futures-fundamentals-refresh-mapping.v1` 声明品种、查询 ID、标准化结果路径和观察日期路径，再生成待复核的 `futures-fundamentals-input.v1`。映射器必须校验刷新内容哈希、查询状态和查询主体与品种的一致性，并将刷新来源、查询 ID、数据哈希、字段路径和映射版本写入字段血缘。

映射器不允许声明 `VERIFIED` 或 `CONFLICTING`，有效值统一进入 `UNVERIFIED`；路径无值时记录 `MISSING`，不猜测、不使用相邻日期补齐。后续仍需通过 `data-source-refresh-evidence-gate.v1` 的人工字段确认和证据包流程，才能作为期货基本面报告的事实输入。该步骤不选择主力、不生成方向、不创建决策快照，保持 `execution_enabled=false`。

### 3.6B 本地 Web 研究控制台

Web 层只作为本地控制面，不复制研究核心逻辑。默认由标准库 HTTP 服务绑定 `127.0.0.1`，页面资源与 API 一起从仓库提供，数据根目录通过启动参数显式指定。

```text
浏览器
  → /api/health、/api/summary、/api/snapshots
  → /api/resolve-task
  → task_resolution / 本地 JSON 快照
```

API 分为三类：

- `GET /api/health`：返回 `LOCAL_ONLY`、执行关闭、模型/网络调用次数和人工发布边界；
- `GET /api/summary`、`GET /api/snapshots`：只扫描白名单数据目录中的 JSON 元数据，限制文件大小和返回数量，不返回原始行情大表；
- `POST /api/resolve-task`：复用统一任务解析器，保存幂等的 `research-task-resolution.v1`。相同任务 ID 已存在时必须校验内容哈希，不得覆盖。

Web 层不直接触发数据刷新、研究管线、决策快照、模拟结算或公众号发布。页面必须展示研究状态、截止日、待复核状态和执行关闭状态；未来新增动作只能调用已有任务状态机，并沿用主备路由、证据闸门和审计留痕。
