...
数据层：原始公告、期货交易所资料、AKShare、QMT、行业/现货统计和研究资料适配器
...
| QMT | A 股、港股及本地权限可用的国内期货行情、历史数据、交易日历、合约信息、模拟研究和可选回放 | 只用于研究和模拟；通过健康检查确认实际字段覆盖，禁止实盘凭证和自动下单 |
...
数据源不可用时，系统应切换备用来源或降低受影响模块的结论强度。AKShare 与 QMT 数据不一致时，双方数据都保留并记录裁决原因。期货行情、结算、持仓、仓单和合约规则优先回溯交易所公开资料，不能假设 AKShare 或 QMT 单独覆盖全部现货与产业数据。QMT 的实盘交易能力在系统配置、权限和产品接口层均应禁用。
...
- 完成 `luopan`、`ai-berkshire`、AKShare、QMT、`czsc`、`chan.py` 和交易所公开资料的能力盘点，形成直接复用、适配复用、仅参考和确实缺失清单；
- 建立统一研究对象、公司、公司边界、行业、产品、期货品种、具体合约、连续序列、证据、原始文档、行情快照和研究版本数据结构；
- 建立统一数据源适配器、研究资产适配器和健康检查，接入原始公告、国内期货交易所公开资料、AKShare 与 QMT；
...
- QMT 只用于行情、历史数据、模拟研究和可选回放，不接入实盘自动执行。
...
首期推荐个人本地部署：本地保存研究数据库、行业雷达快照、候选数据和报告，QMT/AKShare/公开来源按需接入，定时任务在本机运行；只有备份、模型调用或用户明确需要时才增加云服务。这样可以把系统的资源消耗从“全市场公司数量”改为“行业雷达数量 + 入选行业公司池数量 + 深研数量”。
...
合计是个人使用的估算区间，不包含已有 Mac、QMT 既有授权、一次性开发、个人时间、电费精确分摊、商业数据采购、微信公众号认证和人工运营。使用 Wind、Choice、聚宽高级数据或其他商业行情后，应把采购费用单独列项，不能用免费数据成本估算替代。
